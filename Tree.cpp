    ...
    double length, double r1, bool is_trunk)
{
    ...
    double r2 = r1 * ((rand() % 100 + 901) / 1000.0) * branch_r_change_cof;
    ...
    while(r1 >= min_r)
    {
        ...
        y_angle_change = (50 + rand() % 50) / 200.0 * process_bending_y;
        if(rand() % 2 == 0)
        {
                   ...
                   (!is_trunk || (is_trunk && (r1 >= min_r_for_branch) && (r1 <= max_r_for_branch))))
                {
                    if(is_trunk)
                    {
                        ...
                        generate(process_start_position.x, process_start_position.y, process_start_position.z, process_y_angle,
                            process_z_angle, length * trunk_segment_length_change, process_radius, false);
                    }
                    process_generated = true;
                    generate(process_start_position.x, process_start_position.y, process_start_position.z, process_y_angle,
...
void Tree::setTrunkProcessBendingZ(double value)
{
    trunk_process_bending_z = value;
}
...
void Tree::setBranchProcessBendingZ(double value)
{
    branch_process_bending_z = value;
}
